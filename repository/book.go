...
//GetNewBook 获取最新更新书籍
func GetNewBook(openID string) BookList {
	data := BookList{}
	// var book model.Book
...
//GetHotBook 获取最新更新书籍
func GetHotBook(openID string) BookList {
	data := BookList{}
	// var book model.Book
	// books := book.GetNewBooks()
	// 把新书放在内存里面而不是随时去查找数据库
	books := common.GetHotBooks()
	if books != nil {
		for _, b := range books {
			dbo := Book{ID: b.ID, Name: b.Name, Chapter: b.Chapter, UpdatedAt: b.UpdatedAt, Author: b.Author, AuthorURL: b.AuthorURL}
			if openID != "" {
	...
	data.Title = fmt.Sprintf("跟读热门小说")
	data.Description = fmt.Sprintf("跟读热门小说列表")
	data.Keywords = fmt.Sprintf("%v,%v", "跟读", "热门小说")
	data.OpenID = openID
